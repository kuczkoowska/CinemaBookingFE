  ...
  withMethods(
    (
      ...
      toggleBlockUser: rxMethod<User>(
        pipe(
          switchMap((user) =>
            userService.toggleBlockUser(user.id).pipe(
              tapResponse({
                next: () => {
                  const newActiveState = !user.isActive;
                  const action = newActiveState ? 'Odblokowano' : 'Zablokowano';
      ...
      promoteToAdmin: rxMethod<number>(
        pipe(
          switchMap((id) =>
            userService.promoteToAdmin(id).pipe(
              tapResponse({
                next: () => {
      ...
      updateMyProfile: rxMethod<UpdateUserDto>(
        pipe(
          tap(() => store.setLoading()),
          switchMap((data) =>
            userService.updateMyData(data).pipe(
              tapResponse({
                next: () => {
                  store.setLoaded();
